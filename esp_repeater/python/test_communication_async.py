...
def send_and_receive_batch(ser1: SerialPort, ser2: SerialPort, commands: List[str], iterations: int):
    ...
    def read_from_serial(ser, read_data):
        while True:
            response = ser.read()
            if response:
    ...
    # Start the reading thread for the second serial port
    read_thread = threading.Thread(target=read_from_serial, args=(ser2, read_data))
    read_thread.start()
    st = time.time()
...
def main():
    ...
    iterations = 1000
    ser1 = SerialPort(port1, baudrate=baudrate1)
    connect(ser1)
